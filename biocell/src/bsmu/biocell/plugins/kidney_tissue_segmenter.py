...
class BiocellKidneyTissueSegmenterPlugin(Plugin):
    ...
    def _enable_gui(self):
        ...
        self._main_window.add_menu_action(AlgorithmsMenu, 'Segment Tissue', self._segment_tissue, Qt.Key_0)
...
class BiocellKidneyTissueSegmenter(QObject):
    ...
    def segment_tissue(self, layered_image_viewer: LayeredImageViewer):
        ...
        layered_image_viewer.data.add_layer_or_modify_pixels(
            'tissue-mask', tissue_mask, FlatImage, tissue_palette)
        ...
        layered_image_viewer.data.add_layer_or_modify_pixels(
            'blue-mask', blue_mask, FlatImage, blue_palette)

        print('\n=== Initial Result ===')
        self._pixel_count_ratio(blue_mask, tissue_mask)

        print('\n=== Blue Only Within the Tissue ===')
        ...
        # st = timer()
        # tissue_pixel_count = cv.countNonZero(tissue_mask)
        # blue_pixel_count = cv.countNonZero(blue_mask)
        # print('cv.countNonZero time: ', timer() - st)
        # blue_to_tissue_ratio = blue_pixel_count / tissue_pixel_count
        # print(f'tissue_pixel_count: {tissue_pixel_count}  blue_pixel_count: {blue_pixel_count}  '
        #       f'ratio: {blue_to_tissue_ratio}')

    def _pixel_count_ratio(self, a: np.ndarray, b: np.ndarray):
        st = timer()
